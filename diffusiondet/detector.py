...
class DiffusionDet(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample_process(self, batched_inputs, backbone_feats, images_whwh, images, clip_denoised=True, do_postprocess=True):
        batch = images_whwh.shape[0]
        shape = (batch, self.num_proposals, 4)
        total_timesteps, sampling_timesteps, eta, objective = self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        ensemble_score, ensemble_label, ensemble_coord = [], [], []
        #lưu một mảng chứa các predict [time_steps, B, num_proposal, 4]
        predict_process = list([None] * 1001)
        x_start = None
        for time, time_next in time_pairs:
            time_cond = torch.full((batch,), time, device=self.device, dtype=torch.long)
            self_cond = x_start if self.self_condition else None
            ...
            # }
            # predict_process.append({
            #     'time_step': time,
            #     'preds': preds,
            #     'outputs_class': outputs_class[-1],
            #     'outputs_coord': outputs_coord[-1]
            # })
            # predict_process[int(time)] = {
            #     'time_step': time,
            #     'preds': preds,
            #     'outputs_class': outputs_class[-1],
            #     'outputs_coord': outputs_coord[-1]
            # }
            predict_process[int(time)] = time
            ####
            pred_noise, x_start = preds.pred_noise, preds.pred_x_start
            ...
            if self.use_ensemble and self.sampling_timesteps > 1:
                ...
                ensemble_coord.append(box_pred_per_image)
        all_results = []
        assert len(predict_process) == 1000, "{}".format((predict_process))
        # print(len(predict_process))
        if self.use_ensemble and self.sampling_timesteps > 1:
            box_pred_per_image = torch.cat(ensemble_coord, dim=0)
            scores_per_image = torch.cat(ensemble_score, dim=0)
            labels_per_image = torch.cat(ensemble_label, dim=0)
            ...
            all_results = []
            # assert len(predict_process) == 1000
            for element in predict_process:
                time, outputs_class, pred_boxes = element
                output = {'pred_logits': outputs_class, 'pred_boxes': outputs_coord}
                box_cls = output["pred_logits"]
                box_pred = output["pred_boxes"]
                results = self.inference(box_cls, box_pred, images.image_sizes)
                all_results.append((time, results))
        if do_postprocess:
            final_results = []
            for results_per_step in all_results:
                processed_results = []
                time, results = results_per_step
                print(time)
                for results_per_image, input_per_image, image_size in zip(results, batched_inputs, images.image_sizes):
                    height = input_per_image.get("height", image_size[0])
                    width = input_per_image.get("width", image_size[1])
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        # print("########################################################################")
        # Demo detection process
        if self.demo:
            print("*********************************************************************")
            return self.ddim_sample_process(batched_inputs, features, images_whwh, images)
        # Prepare Proposals.
